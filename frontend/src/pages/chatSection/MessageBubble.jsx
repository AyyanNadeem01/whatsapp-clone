...
import { FaCheckDouble as FaDoubleCheck } from "react-icons/fa6"; // if you want the WhatsApp-style double check
...
const MessageBubble = ({
  ...
  // Correctly assign classes based on whether it's a user message
  const bubbleClass = isUserMessage ? `chat-end` : `chat-start`;

  const bubbleContentClass = isUserMessage
    ? `chat-bubble md:max-w-[50%] min-w-[130px] ${theme === "dark" ? "bg-green-700 text-white" : "bg-green-200 text-black"
    }`
    : `chat-bubble md:max-w-[50%] min-w-[130px] ${theme === "dark" ? "bg-gray-700 text-white" : "bg-white text-black"
    }`;

  const quickReactions = ["👍", "❤️", "😂", "😮", "😢", "🙏"];

  const handleReact = (emoji) => {
    console.log("Reacting with:", emoji, "on message:", message._id);
  ...
  return (
          ...
          <div className="self-end flex items-center justify-end
            ...
            {isUserMessage && (
              <>
                ...
                {message.messageStatus === "delivered" && <FaDoubleCheck size={12} />}
                {message.messageStatus === "read" && <FaCheckDouble size={12} />}
              </>
          ...
          <div className="absolute top-1 right-1 opacity-0 
           group-hover:opacity-100 transition-opacity z-20">
            <button onClick={() => setShowOptions((prev) => !prev)}
              className={`p1 rounded-full ${theme === "dark" ? "text-white" : "text-gray-800"}`}
            >
          ...
          <div
            ...
            <button
              ...
              className={`p-2 rounded-full 
                ${theme === "dark" ? "bg-[#202c33] hover:bg-[#202c33]/80" : "bg-white hover:bg-gray-100 shadow-lg"}`}>
              <FaSmile
          ...
          {showReactions && (
            <div ref={reactionsMenuRef}
              className={`absolute -top-10 ${isUserMessage ? "right-0" : "left-0"
                } flex items-center bg-[#202c33]/90 
   ...
   ${theme === "dark" ? "bg-[#2a3942]" : "bg-gray-200"} rounded-full px-2 shadow-md`}>
        ...
        {showOptions && (
          <div ref={optionRef}
            className={`absolute top-0 right-1 z-w-36 rounded-xl shadhow-lg py-2 text-sm
